 ...
 * When I construct the TPCLog in startMockSlave, I SPY on it, meaning it's sort of partially mocked. All methods that don't have a stub defined for it will default to its REAL 
 class defined method rather than returning null/doing nothing.
 ...
 * 
 * As per usual, these tests don't come with any warranty and are presented as is. Meaning if you decide to base your own test cases off this setup and modify the file,
 there is no guarantee things will work as intended. Study the existing framework to increase your chances of success.
 * 
 * This test file is part of the KVStore Project
 * (c) CS162 University of California, Berkeley
 * Written by Isaac Tian, May 2014
 */
public class TPCMurderDeathKillStudTest {
  ...
  public void checkBuild() {
    try {
      ...
      fail("Key 'george' not found on rebuild.");
    }
    try {
      assertTrue(slave1.get("riyaz").equals("faizullabhoy"));
    } catch (KVException e) {
      fail("Key 'riyaz' not found on rebuild.");
    }
    try {
      assertTrue(slave1.get("isaac").equals("tian"));
    } catch (KVException e) {
      fail("Key 'isaac' not found on rebuild.");
    }
    try {
      assertTrue(slave1.get("vaishaal").equals("shankar"));
    } catch (KVException e) {
      fail("Key 'vaishaal' not found on rebuild.");
    }
    try {
      assertTrue(slave1.get("nick").equals("chang"));
    } catch (KVException e) {
      fail("Key 'nick' not found on rebuild.");
    }
    try {
      assertTrue(slave1.get("kelvin").equals("chou"));
    } catch (KVException e) {
      fail("Key 'kelvin' not found on rebuild.");
    }
  ...
  class isPutDel1 extends ArgumentMatcher<KVMessage> {
    ...
    public boolean matches(Object msg) {
      ...
      if (m.getMsgType().equals(KVConstants.PUT_REQ) || m.getMsgType().equals(KVConstants.DEL_REQ)) {
        return true;
      }
      return false;
    }
  ...
  class isPutDel2 extends ArgumentMatcher<KVMessage> {
    ...
    public boolean matches(Object msg) {
      KVMessage m = (KVMessage) msg;
      if (m.getMsgType().equals(KVConstants.COMMIT)) {
        return true;
      }
      return false;
    }
  ...
  Answer dieBeforeLog = new Answer() {
    ...
    public Object answer(InvocationOnMock inv) {
      ...
      //System.out.println("DIE BEFORE LOG CALLED");
      Thread rebuild = new Thread(necromancer);
      try {
        Thread.sleep(2 * TPCMaster.TIMEOUT);
  ...
  Answer dieAfterLog = new Answer() {
    @Override
    public Object answer(InvocationOnMock inv) {
      //System.out.println("DIE AFTER LOG CALLED");
      try {
        inv.callRealMethod();
      } catch (Throwable e) {
      }//shouldn't happen
  ...
  @AGTestDetails(points = 2, desc = "Kills the slave during phase 1 after flushing PUT request to log and rebuilds."
                                    + " Checks that the PUT request was aborted.")
  public void testP1DeathAfterLog() {
    try {
      startMockSlave(SLAVE1, 1);
    } catch (Exception e) {
      fail("can't start slave");
    }
    try {
  ...
  //@Test(timeout = 30000)
  @Category(AG_PROJ4_CODE.class)
  @AGTestDetails(points = 2, desc = "Kills the slave during phase 1 before flushing PUT request to log and rebuilds."
                                    + " Checks that the PUT request was aborted.")
  public void testP1DeathBeforeLog() {
    try {
      startMockSlave(SLAVE1, 2);
    } catch (Exception e) {
      System.out.println(e);
      fail("can't start slave");
    }
  ...
  //@Test(timeout = 30000)
  @Category(AG_PROJ4_CODE.class)
  @AGTestDetails(points = 2, desc = "Kills the slave during phase 2 after flushing COMMIT request to log and rebuilds."
                                    + " Checks that operation completes.")
  public void testP2DeathAfterLog() {
    try {
      startMockSlave(SLAVE1, 3);
    } catch (Exception e) {
      fail("can't start slave");
    }
     try {
  ...
  //@Test(timeout = 30000)
  @Category(AG_PROJ4_CODE.class)
  @AGTestDetails(points = 2, desc = "Kills the slave during phase 2 before flushing COMMIT request to log and rebuilds."
                                    + " Checks that operation completes.")
  public void testP2DeathBeforeLog() {
    try {
      startMockSlave(SLAVE1, 4);
    } catch (Exception e) {
      fail("can't start slave");
    }
     try {
